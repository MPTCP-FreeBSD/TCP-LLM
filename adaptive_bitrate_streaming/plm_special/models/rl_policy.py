...
class OfflineRLPolicy(nn.Module):
    def __init__(
        ...
        self.modules_except_plm = nn.ModuleList([  # used to save and load modules except plm
            ...
            self.embed_state1, self.embed_state2, self.embed_state3, self.embed_state4, self.action_head
        ])
        # Open the file in write mode. This will create the file if it doesn't exist or overwrite it if it does.
        with open('rl_policy_output_init.txt', 'a') as file:
    ...
    def forward(self, states, actions, returns, timesteps, attention_mask=None):
        ...
        for i in range(returns_embeddings.shape[1]):
            stacked_input = torch.cat((returns_embeddings[0, i:i + 1], states_embeddings1[0, i:i + 1], states_embeddings2[0, i:i + 1], 
                                       states_embeddings3[0, i:i + 1], states_embeddings4[0, i:i + 1], action_embeddings[0, i:i + 1]), dim=0)
            stacked_inputs.append(stacked_input)
            action_embed_positions[i] = (i + 1) * (2 + 4)
        ...
        # Open the file in write mode. This will create the file if it doesn't exist or overwrite it if it does.
        with open('rl_policy_output_forward.txt', 'a') as file:
    ...
    def sample(self, state, target_return, timestep, **kwargs):
        ...
        state_embeddings = torch.cat([state_embeddings1, state_embeddings2, state_embeddings3, state_embeddings4], dim=1)
